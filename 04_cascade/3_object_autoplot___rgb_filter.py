...
def setup_initial_hardware_state(bot):
    """초기 하드웨어 상태 설정"""
    # LED 초기화
    bot.Ctrl_WQ2812_ALL(0, 0)  # 시작 시 LED OFF
...
# ============================
print("=" * 50)
print("  Defining Sign Detection Functions")
print("=" * 50)
...
def beep_for_sign_detection():
    """
    표지판 감지 시 부저 울리기 (Raspbot 라이브러리 사용)

    참고: 01_Buzzer driver.ipynb
    - bot.Ctrl_BEEP_Switch(1): 부저 ON
    - bot.Ctrl_BEEP_Switch(0): 부저 OFF
    """
    try:
        bot.Ctrl_BEEP_Switch(1)
        time.sleep(0.3)  # 0.3초간 부저
        bot.Ctrl_BEEP_Switch(0)
    except Exception as e:
...
# ============================
print("=" * 50)
print("  STEP 6: Defining Car Control Functions (Gear Motors)")
print("=" * 50)
...
def set_motor_speeds(motor_0, motor_1, motor_2, motor_3):
    ...
    Args:
        motor_0, motor_1: 왼쪽 바퀴 모터 (0, 1번)
        motor_2, motor_3: 오른쪽 바퀴 모터 (2, 3번)

    제어 방식:
        bot.Ctrl_Muto(motor_id, speed)
        - motor_id: 0~3 (모터 번호)
        - speed: -255 ~ 255 (음수=후진, 양수=전진, 0=정지)
    """
    if not mouse_use:
        bot.Ctrl_Muto(0, 0)
        bot.Ctrl_Muto(1, 0)
...
def rotate_servo(servo_id, angle):
    ...
    Args:
        servo_id: 서보 모터 ID
            - 1: 좌우 회전 (카메라 방향, 0~180도)
            - 2: 상하 회전 (카메라 각도, 0~110도)
        angle: 회전 각도
            - Servo 1: 0~180도
            - Servo 2: 0~110도 (물리적 제한)
...
def handle_keyboard_input():
    ...
    """
    global mouse_use, led_state, beep_state  # 전역 변수 사용 선언
...
try:
    while True:
        ...
        if frame_count % 10 == 0:
            print("\n" + "-" * 50)
            print(
                f"Frame: {frame_count} | Motor: {'ON' if mouse_use else 'OFF'} | LED: {'ON' if led_state else 'OFF'} | Beep: {'ON' if beep_state else 'OFF'}"
            )
            print("-" * 50)
