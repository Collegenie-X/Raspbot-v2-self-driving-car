...
> ⭐ YOLO11 + Haar Cascade 하이브리드 시스템  
> 신호등 감지(YOLO11) + 표지판 감지(Haar Cascade) + RGB 필터링 + 라인 트레이싱 통합
...
### 2.1 하이브리드 검출 시스템 (YOLO11 + Haar Cascade)
...
graph TB
    ...
    subgraph "검출 계층 - 우선순위 기반"
        YOLO[⭐ Priority 1<br/>YOLO11 신호등<br/>빨간불/초록불]
        HAAR[⭐ Priority 2<br/>Haar Cascade<br/>STOP/NO_DRIVE]
...
sequenceDiagram
    ...
    participant YOLO as YOLO11 (신호등)
    ...
    Note over YOLO: ⭐ Priority 1: 신호등 (최우선)
    CAM->>YOLO: frame (원본)
    YOLO->>YOLO: YOLO11 추론
    YOLO->>STATE: 빨간불/초록불 감지
...
### 3.3 모듈 3: YOLO11 신호등 감지 (최우선 순위)

#### 3.3.1 YOLO11 동작 원리

```mermaid
flowchart TB
    A[입력 이미지<br/>320x240] --> B[자동 리사이징<br/>640x640]
    
    B --> C[YOLO11<br/>신경망 추론]
    C --> D[백본<br/>특징 추출]
    D --> E[넥<br/>특징 융합]
    E --> F[헤드<br/>객체 검출]
    
    F --> G[Bounding Box<br/>예측]
    F --> H[클래스 분류<br/>3개 클래스]
    F --> I[신뢰도 계산<br/>Confidence]
    
    G --> J[NMS 처리<br/>IOU threshold]
    H --> J
    I --> J
    
    J --> K{신뢰도 ><br/>threshold?}
    
    K -->|No| L[필터링]
    K -->|Yes| M[최종 검출]
    
    M --> N{클래스 판별}
    N -->|0| O[traffic_red_light<br/>빨간불]
    N -->|1| P[traffic_green_light<br/>초록불]
    N -->|2| Q[traffic_light<br/>일반 신호등]
...
| 파라미터 | 값 | 효과 | 트레이드오프 |
|:---|:---:|:---|:---|
| **confidence** | 0.5 (권장) | 검출 신뢰도 임계값 | ↓ 0.3: 오탐 증가<br/>↑ 0.7: 미검출 증가 |
| **iou** | 0.45 (권장) | NMS 중복 제거 임계값 | ↓ 0.3: 중복 증가<br/>↑ 0.6: 놓칠 수 있음 |
| **입력 크기** | 640x640 | 자동 리사이징 | 큰 이미지: 느리지만 정확 |
| **클래스 수** | 3 | 빨강/초록/일반 | 커스텀 모델 전용 |
...
yolo_model = YOLO("./models/traffic_modeln.pt", verbose=False)
...
def detect_traffic_lights_yolo(frame, confidence_threshold=0.5, iou_threshold=0.45):
    ...
    권장 설정:
        - confidence: 0.5 (일반), 0.4 (민감), 0.6 (엄격)
        - iou: 0.45 (중복 제거 적절)
    """
    # Early Return: YOLO 모델 없음
    if yolo_model is None:
        ...
        cv2.putText(annotated, "YOLO MODEL NOT LOADED", (10, 30),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
        return False, False, annotated, empty_info
...
### 4.1 우선순위 기반 처리 (YOLO11 + Haar Cascade + Line Tracing)

#### 4.1.1 알고리즘 설명

**3단계 우선순위 시스템**은 신호등(YOLO11)을 **최우선**, 표지판(Haar Cascade)을 **차순위**, 자율주행을 **기본**으로 처리하는 구조입니다.

**주요 특징**:
- ⭐ Priority 1: 신호등 (YOLO11) - 빨간불 시 모든 동작 정지
...
while True:
    ...
    # ═══════════════════════════════════════
    # ⭐ Priority 1: 신호등 감지 (YOLO11 - 최우선)
    # ═══════════════════════════════════════
    red_detected, green_detected, info = detect_traffic_lights_yolo(frame)
...
### 5.1 메인 루프 통합 코드 (YOLO11 + Haar Cascade 하이브리드)

```python
# ========================================
# 메인 실행 코드 (v3.0 - YOLO11 통합)
# ========================================

def main():
    """메인 실행 함수 - 하이브리드 검출 시스템"""
    
    # ══════════════════════════════════════
    # 1단계: 초기화
    # ══════════════════════════════════════
    print("=" * 50)
    print("  Raspbot v2 자율주행 시스템 시작 (v3.0)")
    print("  YOLO11 + Haar Cascade 하이브리드")
    print("=" * 50)
    ...
    params = {
        ...
        "motor_up_speed": 15,
        "motor_down_speed": 8,
        "yolo_confidence": 0.5,  # YOLO 신뢰도
        "yolo_iou": 0.45,        # YOLO IOU
