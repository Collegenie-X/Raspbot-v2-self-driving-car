# "자율주행 가상 시뮬레이션 제작" 과제 (print문으로 작성) 

## 📋 과제 개요

**목표**: **자율주행 시스템의 가상 시뮬레이션을 설계하여 단순하게 print문으로 출력하여, 실제 개발에 참고하기 위함입니다. 
         자율 주행에서 발생하는 사건 사고들을(주차장,신호등,표지판...) 정의하여,가상환경을 만들고 조건에 따른 제어를 print문으로 출력하면 됩니다. 
...
**핵심 방식**:
- 각 5개의 TODO에서 **인식 → 즉시 제어 결정** (제어 시스템 따로 분리 없음)
- 인식 결과에 따라 기어모터, 서보모터, 부저 제어하는 부분을 출력
- 20 Frames 반복하여 다양한 상황 시뮬레이션 반복하여 자율 주행함. 
