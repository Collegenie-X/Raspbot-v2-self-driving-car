...
> **최종 업데이트**: 2025-12-16 (v3.0 - YOLO11 통합)  
> **구현 파일**: `1_yolo_final_autoplot.py`

## 📋 1. 개요
본 문서는 자율주행 챌린지 미션 영상을 기반으로, **Raspbot v2 플랫폼**에서 동일한 기능을 구현하기 위한 요구사항을 분석하고 정리한 문서입니다.

### 1.1 미션 목표
- 🚦 신호등 인식 및 출발 제어 ⭐ **YOLO11 적용**
- 🛣️ 차선 유지 주행 (직선 + 곡선)
- 🚧 장애물 감지 및 회피
...
### 1.1.1 구현 시스템 구조 (하이브리드)

```
┌─────────────────────────────────────────┐
│   YOLO11 + Haar Cascade 하이브리드      │
├─────────────────────────────────────────┤
│ ⭐ Priority 1: 신호등 (YOLO11)         │
│   - traffic_red_light (빨간불)          │
│   - traffic_green_light (초록불)        │
│   - traffic_light (일반)                │
├─────────────────────────────────────────┤
...
**기존 계획**:
- 신호등: HSV 색상 필터링 + Haar Cascade

**현재 구현**:
- ⭐ 신호등: **YOLO11** (빨간불/초록불 정확 구분)
...
### 5.1 신호등 대기 및 출발 ⭐ (YOLO11 적용)

#### 전체 인식 프로세스 (YOLO11 기반)
```mermaid
flowchart TD
    A[카메라 프레임<br/>320x240] --> B[YOLO11<br/>신경망 추론]
    
    B --> C[자동 리사이징<br/>640x640]
    C --> D[특징 추출<br/>백본 네트워크]
    D --> E[객체 검출<br/>헤드 네트워크]
    
    E --> F[3개 클래스<br/>동시 검출]
    
    F --> G[Class 0:<br/>traffic_red_light]
    F --> H[Class 1:<br/>traffic_green_light]
    F --> I[Class 2:<br/>traffic_light]
    
    G --> J{Confidence<br/>> 0.5?}
    H --> K{Confidence<br/>> 0.5?}
    I --> L{Confidence<br/>> 0.5?}
    ...
    M --> Q{현재 상태}
    Q -->|대기 중 아님| R[waiting_for_green = True]
    Q -->|이미 대기 중| S[상태 유지]
    ...
    style M fill:#ff6b6b
    style O fill:#51cf66
    style P fill:#d3d3d3,color:#111
    style Z fill:#51cf66
```

#### YOLO11 vs 기존 방식 비교

| 구분 | 기존 방식 (HSV) | YOLO11 (현재 구현) |
|:---|:---|:---|
| **정확도** | 60-70% | 90-95% |
| **빨강/초록 구분** | 픽셀 카운팅 (부정확) | 클래스별 학습 (정확) |
| **조명 영향** | 매우 큼 | 적음 |
| **거리 변화 대응** | 약함 | 강함 |
| **처리 속도** | 빠름 (5ms) | 느림 (50-100ms) |
| **오탐률** | 높음 (20-30%) | 낮음 (5-10%) |
| **구현 난이도** | 쉬움 | 어려움 (학습 필요) |
| **하드웨어 요구** | 낮음 | 높음 (GPU 권장) |

#### 단계별 상세 알고리즘 (YOLO11 기반)

##### 1단계: YOLO11 추론
```mermaid
flowchart LR
    A[입력 프레임<br/>320x240] --> B[전처리<br/>Resize to 640x640]
    B --> C[정규화<br/>0-1 범위]
    C --> D[YOLO11 백본<br/>특징 추출]
    D --> E[YOLO11 넥<br/>특징 융합]
    E --> F[YOLO11 헤드<br/>검출]
    F --> G[NMS 처리<br/>IOU=0.45]
    G --> H[최종 검출<br/>Confidence>0.5]
```

##### 2단계: 클래스 판별 및 제어
```mermaid
flowchart TD
    A[YOLO 검출 결과] --> B{검출된 클래스}
    
    B -->|Class 0<br/>red_light| C[빨간불 처리]
    B -->|Class 1<br/>green_light| D[초록불 처리]
    B -->|Class 2<br/>traffic_light| E[일반 신호등<br/>참고용]
    B -->|검출 없음| F[자율주행 계속]
    
    C --> C1{현재<br/>대기 중?}
    C1 -->|No| C2[waiting_for_green = True]
    C1 -->|Yes| C3[상태 유지]
    C2 --> C4[부저 1회 울림]
    C4 --> C5[모터 정지]
    C3 --> C5
...
| 파라미터 | 값 | 설명 |
|:---|:---:|:---|
| **모델 파일** | traffic_modeln.pt | 커스텀 학습 모델 |
| **입력 크기** | 640x640 | 자동 리사이징 |
| **Confidence** | 0.5 | 검출 신뢰도 임계값 |
| **IOU** | 0.45 | NMS 중복 제거 임계값 |
| **클래스 수** | 3 | red/green/general |
| **추론 시간** | 50-100ms | Raspberry Pi 4 기준 |
| **정확도** | 90-95% | 테스트 데이터 기준 |
...
### 7.0 메인 루프 전체 흐름도 (YOLO11 + Haar Cascade)
```mermaid
flowchart TD
    A[시스템 시작] --> B[하드웨어 초기화]
    B --> B1[⭐ YOLO11 모델 로드<br/>traffic_modeln.pt]
    ...
    E1 --> F[⭐ Priority 1: YOLO11 신호등]
    F --> G{신호등 검출?}
    
    G -->|빨간불| H1[waiting_for_green = True]
    H1 --> H2{부저<br/>재생?}
    H2 -->|No| H3[부저 1회 울림]
    H2 -->|Yes| H4
    H3 --> H4[모든 기능 정지]
    H4 --> H5[⏳ 초록불 대기]
    H5 --> SKIP_ALL[표지판/차선 스킵]
    
    G -->|초록불 + 대기 중| I1[모든 상태 리셋]
    I1 --> I2[부저 1회 울림]
    I2 --> I3[✅ 자율주행 재개]
    
    G -->|검출 없음| J[⭐ Priority 2: Haar 표지판]
    I3 --> J
    
    J --> J1{신호등<br/>대기 중?}
    J1 -->|Yes| SKIP_SIGN[표지판 감지 스킵]
    J1 -->|No| K[선택된 프레임 소스로<br/>Cascade 검출]
...
### 7.0.1 우선순위 처리 로직 (YOLO11 + Haar Cascade)
```mermaid
flowchart LR
    A[센서 입력] --> B{우선순위 판단}
    
    B -->|P1 최고| C[⭐ 신호등<br/>YOLO11]
    B -->|P2 높음| D[⭐ 표지판<br/>Haar Cascade]
    B -->|P3 중간| E[장애물<br/>초음파]
    B -->|P4 낮음| F[차선 추종<br/>히스토그램]
    
    C --> C1{빨간불?}
    C1 -->|Yes| G[모든 기능 정지<br/>초록불 대기]
    C1 -->|No| C2{초록불?}
    C2 -->|Yes| H[모든 상태 리셋<br/>자율주행 재개]
    C2 -->|No| I[자율주행 계속]
...
| 항목 | 기존 (v2.0) | 현재 (v3.0) |
|:---|:---|:---|
| **감지 방식** | Haar Cascade + HSV | ⭐ **YOLO11** |
| **정확도** | 60-70% | 90-95% |
| **빨강/초록 구분** | 픽셀 카운팅 (부정확) | 클래스별 학습 (정확) |
| **모델 파일** | traffic_light.xml | traffic_modeln.pt |
| **추론 시간** | 5ms | 50-100ms |
| **오탐률** | 20-30% | 5-10% |
...
```python
YOLO_CLASS_NAMES = {
    0: "traffic_red_light",     # 빨간불
    1: "traffic_green_light",   # 초록불
    2: "traffic_light",         # 일반 신호등
}
```
...
| 항목 | 측정값 |
|:---|:---:|
| **YOLO11 추론 시간** | 50-100ms |
| **Haar Cascade 추론** | 5-10ms |
| **히스토그램 분석** | 2-3ms |
| **전체 루프 시간** | 60-120ms |
| **실시간 FPS** | 8-16 FPS |
...
**최종 수정일**: 2025-12-16  
**작성자**: Raspbot v2 개발팀  
**주요 업데이트**: YOLO11 통합, 우선순위 시스템 재구성
