...
- **파일명 예시**: `autoplot_simulation_학번_이름.py`  
  (예: autoplot_simulation_1234567_홍길동.py)
...
```mermaid
flowchart TD
    A([Start<br/>주차공간 O 표시 인식 & 제어])
    B{O 표시 위치는?}
    C1[인식: left<br/>기어모터: 좌회전]
    C2[인식: center<br/>주차 위치 확인]
    C3[인식: right<br/>기어모터: 우회전]
    C4[인식: fail<br/>기어모터: 전진 탐색]
    D{크기: big/small?}
    E1[big - 가까움<br/>기어모터: 후진 주차]
    E2[small - 멀다<br/>기어모터: 전진 접근]
