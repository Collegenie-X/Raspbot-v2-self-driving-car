...
# -*- coding: utf-8 -*-
"""
Raspbot v2 최종 통합 자율주행 시스템 (Final Autonomous Driving System)
신호등 제어 + 표지판 감지 + 자율주행 통합 버전

Created: 2025-12-15 (v2.0 - 최종 통합)
...
- ⭐ 신호등 감지: 빨간불/초록불 (최우선 순위)
- ⭐ 표지판 감지: Stop sign, No Drive sign (차순위)
...
═══════════════════════════════════════════════════════════
1. 신호등 제어 (최우선):
   - 🔴 RED 신호등: 모터 정지 → GREEN 신호등까지 대기
   - 🟢 GREEN 신호등: 모든 정지 상태 해제 → 자율주행 재개

2. 표지판 감지 (차순위):
   - 🛑 STOP 표지판: 모터 정지 → 표지판 사라질 때까지 대기
   - 🚫 NO DRIVE 표지판: 모터 정지 → 표지판 사라질 때까지 대기

3. 자율주행 (기본):
   - ✅ 신호/표지판 없음: 라인 트레이싱 자율주행

상태 전환 로직:
═══════════════════════════════════════════════════════════
[정상 주행]
    ↓
[RED 신호등 감지] → 정지 상태 진입 → waiting_for_green = True
    ↓
[RED 사라짐] → 정지 상태 계속 유지 (⭐ GREEN 대기)
    ↓
[GREEN 신호등 감지] → 모든 상태 리셋 → 자율주행 재개
    ↓
[표지판 감지] → 정지 상태 진입 → 표지판 사라질 때까지
    ↓
[표지판 사라짐] → 즉시 자율주행 재개
...
═══════════════════════════════════════════════════════════
- ./xml/stop.xml (정지 표지판)
- ./xml/no_drive.xml (통행금지 표지판)
- ./xml/stop.xml (빨간불 신호등 - 동일 모델 사용)
- ./xml/no_drive.xml (초록불 신호등 - 동일 모델 사용)
"""
...
print("⭐ Traffic Light Control + Sign Detection System")
print("   - Priority 1: Traffic Light (RED/GREEN)")
print("   - Priority 2: Traffic Sign (STOP/NO DRIVE)")
...
red_light_cascade_path = "./xml/traffic_red_light.xml"  # 빨간불 (stop.xml 사용)
green_light_cascade_path = "./xml/traffic_green_light.xml"  # 초록불 (no_drive.xml 사용)
...
if stop_cascade.empty():
    ...
    print("✅ stop.xml loaded successfully (STOP sign)")

if no_drive_cascade.empty():
    print("⚠️  Warning: no_drive.xml not found")
else:
    print("✅ no_drive.xml loaded successfully (NO DRIVE sign)")

# 신호등 모델 확인
if red_light_cascade.empty():
    print("⚠️  Warning: red_light.xml not found (using stop.xml)")
else:
    print("✅ red_light.xml loaded successfully (RED light)")

if green_light_cascade.empty():
    print("⚠️  Warning: green_light.xml not found (using no_drive.xml)")
else:
    print("✅ green_light.xml loaded successfully (GREEN light)")
...
def visualize_direction_on_frame(
    ...
    Args:
        ...
        traffic_light_stop: 신호등으로 인한 정지 상태 (NEW)
        sign_stop: 표지판으로 인한 정지 상태 (NEW)
    """
    # 컬러 이미지로 변환
...
# ============================
print("=" * 50)
print("  Defining Traffic Light and Sign Detection Functions")
print("=" * 50)
...
def detect_traffic_signs(
    ...
    # 표지판 상태 표시 (상단)
    if stop_detected:
        status_text = "SIGN: STOP"
        status_color = (0, 0, 255)
    elif no_drive_detected:
        status_text = "SIGN: NO DRIVE"
...
print("Traffic Light and Sign detection functions defined successfully\n")
...
print("=" * 50)
print("⭐ Final Integrated Autonomous Driving System:")
print("  🔴 RED Light → Motor STOP (wait for GREEN)")
print("  🟢 GREEN Light → Release STOP state, Resume AUTO DRIVE")
print("  🛑 STOP Sign → Motor STOP (until sign disappears)")
print("  🚫 NO DRIVE Sign → Motor STOP (until sign disappears)")
print("  ✅ No Signal → Auto Driving (Line Tracing)")
print("=" * 50)
print("⭐ Priority:")
print("  1. Traffic Light (RED/GREEN) - Highest")
print("  2. Traffic Sign (STOP/NO DRIVE) - Middle")
    ...
    while True:
        ...
        if frame_count % 10 == 0:
            ...
            if waiting_for_green:
                if red_light_active:
                    print("🔴 State: RED LIGHT detected - MOTOR STOPPED")
                else:
                ...
                print(f"🛑 State: {sign_type} sign detected - MOTOR STOPPED")
            else:
        ...
        # 3) gray_rgb_frame: RGB 가중치 기반 그레이
        ...
        if not waiting_for_green:
            ...
            if stop_detected:
                ...
                if not stop_sign_active:
                    ...
                    if DEBUG_MODE:
                        print(f"\n{'='*50}")
                        print(
                            f"🛑 STOP sign DETECTED! Position: {sign_info['object_position']}"
                        )
                        print(f"{'='*50}")
            ...
            if no_drive_detected:
                ...
                if not no_drive_sign_active:
                    ...
                    if DEBUG_MODE:
                        print(f"\n{'='*50}")
                        print(
                            f"🚫 NO DRIVE sign DETECTED! Position: {sign_info['object_position']}"
                        )
                        print(f"{'='*50}")
            ...
            # 신호등 대기 중이면 표지판 감지 화면만 표시 (빈 프레임)
